...
def get_usps(train, transform, path, image_size=28, num_channel=3, batch_size=32, in_memory=True, is_balanced=False,
             drop_last=True, download=True):
    """Get USPS dataset loader."""
    # dataset and data loader
    usps_dataset = USPS(root=f"{path}/data/",
                        train=train,
                        transform=transform,
                        download=download)
    ...
    if in_memory:
        ...
        if is_balanced:
            usps_data_loader = torch.utils.data.DataLoader(
                dataset=usps_dataset,
                batch_size=batch_size,
                sampler=BalancedBatchSampler(usps_dataset),
                drop_last=drop_last)
        else:
            usps_data_loader = torch.utils.data.DataLoader(
                dataset=usps_dataset,
